module.exports = function asyncFileUpload() {
  ...
  function handleSuccess(res) {
    ...
    Object.keys(success).forEach(function(key) {
      ...
      window.jQuery(`input[name="${key}"]`).val(success[key].hashName);
    });
  ...
  function handleFailure() {
    const $progressContainer = window.jQuery(this).next(".progress.mt-1");
  ...
  $inputs.each(function(idx, $input) {
    ...
    ow.jQuery($input)
      ...
      .click(function(e) {
        e.preventDefault();
        const hash = window.jQuery(this).attr("destroy-file");
        const $inputFile = window.jQuery($input);
